...
fun findCorrespondingSupertype(
    ...
    while (!queue.isEmpty()) {
        ...
        if (typeCheckingProcedureCallbacks.assertEqualTypeConstructors(constructor, supertypeConstructor)) {
            ...
            val constructor1 = substituted.constructor
            if (!typeCheckingProcedureCallbacks.assertEqualTypeConstructors(constructor1, supertypeConstructor)) {
                throw AssertionError("Type constructors should be equals!\n" +
                                     "substitutedSuperType: ${constructor1.debugInfo()}, " +
                                     "supertype: ${supertypeConstructor.debugInfo()}" +
                                     typeCheckingProcedureCallbacks.assertEqualTypeConstructors(constructor1, supertypeConstructor))
            }
