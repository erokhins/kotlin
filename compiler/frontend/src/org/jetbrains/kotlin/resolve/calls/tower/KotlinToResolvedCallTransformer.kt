...
class KotlinToResolvedCallTransformer(
    ...
    fun <D : CallableDescriptor> transformAndReport(
        ...
        val candidate = baseResolvedCall.resultCall!!
        when (baseResolvedCall.type) {
            CallResolutionResult.Type.PARTIAL -> {
                context.trace.record(BindingContext.ONLY_RESOLVED_CALL, candidate.ktPrimitive.psiKotlinCall.psiCall, baseResolvedCall)
            ...
            CallResolutionResult.Type.ERROR, CallResolutionResult.Type.COMPLETED -> {
                ...
                for (subKtPrimitive in candidate.subKtPrimitives) {
...
class NewResolvedCallImpl<D : CallableDescriptor>(
    ...
    private val resultingDescriptor = run {
        ...
        val containsCapturedTypes = resolvedCall.candidateDescriptor.returnType?.contains { it is NewCapturedType } ?: false
        ...
        when {
            ...
            (candidateDescriptor is PropertyDescriptor && (candidateDescriptor.typeParameters.isNotEmpty() || containsCapturedTypes)) ->
                ...
                candidateDescriptor.substitute(resolvedCall.substitutor).substituteAndApproximateCapturedTypes(substitutor)
            else ->
                candidateDescriptor
        }
    }

    val typeArguments = resolvedCall.substitutor.freshVariables.map {
    ...
    override val kotlinCall: KotlinCall get() = resolvedCall.ktPrimitive

    override fun getStatus(): ResolutionStatus = getResultApplicability(resolvedCall.diagnostics).toResolutionStatus()

    override val argumentMappingByOriginal: Map<ValueParameterDescriptor, ResolvedCallArgument>
        get() = resolvedCall.argumentMappingByOriginal

    override fun getCandidateDescriptor(): D = resolvedCall.candidateDescriptor as D
    override fun getResultingDescriptor(): D = resultingDescriptor as D
    override fun getExtensionReceiver(): ReceiverValue? = resolvedCall.extensionReceiverArgument?.receiver?.receiverValue
    override fun getDispatchReceiver(): ReceiverValue? = resolvedCall.dispatchReceiverArgument?.receiver?.receiverValue
    override fun getExplicitReceiverKind(): ExplicitReceiverKind = resolvedCall.explicitReceiverKind
