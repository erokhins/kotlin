...
class KtPrimitiveCompleter(
    ...
    fun completeAndReport(resolvedKtPrimitive: ResolvedKtPrimitive) {
        when (resolvedKtPrimitive) {
            is ResolvedKtCollectionLiteral -> completeCollectionLiteralCalls(resolvedKtPrimitive)
            is ResolvedKtCallableReference -> completeCallableReference(resolvedKtPrimitive)
            is ResolvedKtLambda -> completeLambda(resolvedKtPrimitive)
            is ResolvedKtCall -> completeResolvedCall(resolvedKtPrimitive)
        }
    }

    fun completeAll(resolvedKtPrimitive: ResolvedKtPrimitive) {
        for (subKtPrimitive in resolvedKtPrimitive.subKtPrimitives) {
            completeAll(subKtPrimitive)
        }
        completeAndReport(resolvedKtPrimitive)
    }
    ...
    private fun completeLambda(lambdaArgument: ResolvedKtLambda) {
            val returnType = resultSubstitutor.safeSubstitute(lambdaArgument.returnType)
    ...
    private fun updateTraceForLambdaReturnType(lambdaArgument: ResolvedKtLambda, trace: BindingTrace, returnType: UnwrappedType) {
        val psiCallArgument = lambdaArgument.ktPrimitive.psiCallArgument
    ...
    private fun completeCallableReference(
            callableReferenceArgument: ResolvedKtCallableReference
    ) {
        val callableCandidate = callableReferenceArgument.candidate
        ...
        val psiCallArgument = callableReferenceArgument.ktPrimitive.psiCallArgument as CallableReferenceKotlinCallArgumentImpl
    ...
    private fun completeCollectionLiteralCalls(collectionLiteralArgument: ResolvedKtCollectionLiteral) {
        val psiCallArgument = collectionLiteralArgument.ktPrimitive.psiCallArgument as CollectionLiteralKotlinCallArgumentImpl
