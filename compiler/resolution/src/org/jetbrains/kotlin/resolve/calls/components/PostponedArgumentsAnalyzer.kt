...
class PostponedArgumentsAnalyzer(
    ...
    fun analyze(c: Context, resolutionCallbacks: KotlinResolutionCallbacks, argument: ResolvedKtPrimitive) {
        when (argument) {
            is ResolvedKtLambda -> analyzeLambda(c, resolutionCallbacks, argument)
            is ResolvedKtCallableReference -> callableReferenceResolver.processCallableReferenceArgument(c.getBuilder(), argument)
            is ResolvedKtCollectionLiteral -> TODO("Not supported")
            else -> error("Unexpected resolved primitive: ${argument.javaClass.canonicalName}")
        }
    }

    private fun analyzeLambda(c: Context, resolutionCallbacks: KotlinResolutionCallbacks, lambda: ResolvedKtLambda) {
        ...
        val resultArguments = resolutionCallbacks.analyzeAndGetLambdaResultArguments(lambda.ktPrimitive, lambda.isSuspend, receiver, parameters, expectedType)
